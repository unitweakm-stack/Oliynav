...
class SlideGenerator:
    ...
    def generate_content(self, topic):
        prompt = f"Mavzu: {topic}. Institut talabalari uchun 6-8 ta slayddan iborat mazmun yarat. Javobni faqat JSON formatida qaytar: [{{'title': '...', 'content': '...'}}]. Til: O'zbekcha."
        try:
    ...
    def create_pptx(self, topic, content_list, output_file):
        prs = Presentation()
        # Title Slide
        slide = prs.slides.add_slide(prs.slide_layouts[0])
        slide.shapes.title.text = topic
        slide.placeholders[1].text = "Institut talabalari uchun maxsus\nGemini AI orqali tayyorlandi"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_data[user_id] = {"auth": False, "count": 0}
    await update.message.reply_text("🎓 Institut slayd botiga xush kelibsiz!\nIshlatish uchun kalitni (KEY) kiriting:")

async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    text = update.message.text
    if user_id not in user_data: user_data[user_id] = {"auth": False, "count": 0}

    if not user_data[user_id]["auth"]:
        if text in VALID_KEYS:
            user_data[user_id]["auth"] = True
            await update.message.reply_text("✅ Kalit tasdiqlandi! Endi slayd mavzusini yuboring:")
        else:
            await update.message.reply_text("❌ Noto'g'ri kalit!")
        return

    # Slayd yaratish
    msg = await update.message.reply_text("⏳ Gemini AI slayd tayyorlamoqda, iltimos kuting...")
    generator = SlideGenerator(GEMINI_API_KEY)
    content = generator.generate_content(text)
    file_path = f"slide_{user_id}.pptx"
    generator.create_pptx(text, content, file_path)

    try:
        with open(file_path, 'rb') as f:
            await context.bot.send_document(chat_id=user_id, document=f, filename=f"{text}.pptx", caption=f"✅ {text} mavzusidagi slaydingiz tayyor!")
        ...
        if user_data[user_id]["count"] >= 2:
            ...
            await context.bot.send_message(chat_id=user_id, text="Yana 1 ta slayd yaratishingiz mumkin. Keyingi mavzuni yuboring:")
...
if __name__ == '__main__':
    if not BOT_TOKEN: sys.exit("BOT_TOKEN yo'q!")
